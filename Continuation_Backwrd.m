...
Omega=Om_stp; %OmeGa=3.52; %Omeg_0+eps*b2;                   % Frequency interval for sweeping
etta_0=[1 0.01 0 0]; %1e-5*ones(1,2*N);
x0(1:2*N)=etta_0;

%%% Computing the starting point solution using Shooting scheme
d_etta=ones(1,2*N);
monodromy_Mat=zeros(2*N,2*N); rondx_rondeta=monodromy_Mat;
...
if abs(Eig_val)<1.001
    ...
    %plot(omeg(j),eigen(j),'b.','MarkerSize',7);
    hold on
else
    plot(omeg(j),Wmax(j),'r.','MarkerSize',7);
    %plot(omeg(j),eigen(j),'r.','MarkerSize',7);
    hold on  
end
X_init(j,:)=x0(1:2*N);
j=j+1
...
tic
while Omega > Om_st %OmeGa > Omeg_0+eps*b1
